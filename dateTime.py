...
class DateTime:
    ...
    def addHoursToDateTime(self, dateTime, hours):
        """
    ...
    def biggestDate(self, dateTime1, dateTime2):
        """
    ...
    def updateHours(self, time):    
        min = self.minutesToInt(time)
        h = self.hourToInt(time)
        min=min +30
        if min>=60:
            h=h+1
            min=min-60
    ...
    def computeNewTimes(self, scheduleTime, scheduleDay):
        newScheduleTime = self.updateHours(scheduleTime)
        if( newScheduleTime[:2] == "20"):
            newScheduleTime = "04h00"
            newScheduleDay = self.updateDay(scheduleDay)
        else:
